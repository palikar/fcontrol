...
processors['message'] = function(event, api){
    ...
    cache_update(event, api, (event) => {
        
        if (event.senderID == userID) {
            logging.log_personal(event.threadID + ' ; ' + user_info_cache[event.senderID]['name'] + ' ; ' + event.body);
            saver.save_attachment_personal(event.attachments, event, api);
	} else {   
	    logging.log_msg(event.threadID+' ; '+user_info_cache[event.senderID]['name']+' ; '+event.body);
...
processors['typ'] = function(event, api){
	...
	if(event.userInfo !== undefined){
	    logging.log_presence(event.threadID + ' ; '+event.userInfo['name']+' ;typing; '+ event.isTyping);
	}
    });
};
...
processors['presence'] = function(event, api){
	...
	if(event.userInfo !== undefined){
	    ...
	    logging.log_presence(event.userInfo['name']+' ; presence ; '+stat);
	}
